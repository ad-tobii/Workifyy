...
const ReviewModal = ({ isOpen, onClose, jobId, professionalName }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'We could not submit your review. Please try again.')
      setAllowRetry(true)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-3xl bg-[#131314] p-6 shadow-2xl">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-xl font-bold text-white">{success ? 'Review Submitted!' : 'Accept Work & Review'}</h2>
          <button
            onClick={handleClose}
        ...
        {success ? (
          ...
          <>
            <p className="mb-6 text-center text-sm text-zinc-400">
              Rate your experience with <span className="font-semibold text-white">{professionalName}</span>
            </p>
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                className="w-full resize-none rounded-2xl border border-zinc-800 bg-zinc-900 p-4 text-sm text-white outline-none placeholder:text-zinc-500 focus:border-[#32cd32] disabled:opacity-60"
              />
            </div>
            ...
            {allowRetry ? (
              <button
                ...
                disabled={loading}
                className="w-full rounded-2xl bg-[#32cd32] py-3 text-sm font-bold text-black transition-all hover:bg-[#2eb32e] active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
              >
